...
async def handle_web_app_data(message: Message):
    ...
    if data['action'] == 'deposit':
        ...
        # СОЗДАЁМ НАСТОЯЩИЙ СЧЁТ (как на фото)
        await message.answer_invoice(
            title="Пополнение баланса Stars Arena",
            description=f"Пополнение игрового счета на {amount} звезд",
            payload=f"deposit_{message.from_user.id}_{amount}",
            currency="XTR",
            prices=[
                LabeledPrice(label="Пополнение баланса", amount=amount)
            ],
            provider_token=""
        )
        ...
        winnable = db.get_winnable_balance(message.from_user.id)

        if amount > winnable:
            await message.answer("❌ Можно выводить только выигранные звезды")
            return
        ...
        if result['success']:
            await message.answer(
                f"✅ Заявка на вывод #{result['request_id']} создана!\n"
                f"Сумма: {amount}⭐\n"
            ...
            await bot.send_message(
                ...
                text=(
                    f"🚨 НОВАЯ ЗАЯВКА #{result['request_id']}\n"
...
async def show_profile(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐ Пополнить", web_app=WebAppInfo(url=MINI_APP_URL))],
...
async def withdraw_menu(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💸 Открыть Mini App для вывода", web_app=WebAppInfo(url=MINI_APP_URL))],
